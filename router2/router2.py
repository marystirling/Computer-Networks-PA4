...
def driver (args):
  ...
  try:
    print("Look for next hop in routing table")
    f = open("routing_table.json")
    routing_table = json.load(f)
    f.close()
    print(routing_table)
    if "msb_router2" in routing_table:
      args.myaddr, args.myport = routing_table["msb_router2"][0]
      args.nexthopaddr, args.nexthopport = routing_table["msb_router2"][2]

  except:
    print ("Some exception occurred looking at router table {}".format (sys.exc_info()[0]))
    return
